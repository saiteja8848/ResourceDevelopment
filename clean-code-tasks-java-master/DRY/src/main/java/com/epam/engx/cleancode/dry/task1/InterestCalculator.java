...
public class InterestCalculator {
    ...
    private BigDecimal interest(AccountDetails accountDetails) {
        ...
        if (isAccountStartedAfterBonusAge(accountDetails)) 
        {
            if (AGE <= accountDetails.getAge()) {
                ...
                interest =InterestByAge(accountDetails) * SENIOR_PERCENT / 100;
            } else {
                interest = InterestByAge(accountDetails) * INTEREST_PERCENT / 100;
            }
        }
    ...
    double InterestByAge(AccountDetails accountDetails)
    {
